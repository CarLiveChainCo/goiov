...
import (
	...
	"github.com/carlivechain/goiov/consensus/clique"
	"github.com/carlivechain/goiov/consensus/ethash"
	"github.com/carlivechain/goiov/core"
	"github.com/carlivechain/goiov/core/bloombits"
	...
	"github.com/carlivechain/goiov/eth/filters"
...
func New(ctx *node.ServiceContext, config *Config, nodeCfg *node.Config) (*Ethereum, error) {
	...
	if mainAlien, ok := eth.engine.(*alien.Alien); ok {
		mainAlien.SetEth(eth)
	}
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db ethdb.Database, testFlag ...bool) consensus.Engine {
	...
	if chainConfig.Clique != nil {
		...
		return alien.New(chainConfig.Alien, db, flag)
	}
...
func (s *Ethereum) StartMining(local bool, id string) error {
	...
	if id != "" {
		if alien, ok := s.sideChains[id].Engine().(*alien.Alien); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
